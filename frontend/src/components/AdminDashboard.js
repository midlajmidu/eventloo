...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setTeamsData(Array.isArray(teamsResponse.data) ? teamsResponse.data : teamsResponse.data?.results || []);
  ...
  const handleManageMembers = (team) => {
    // Find the full team data from teamsData for member management
    const fullTeamData = teamsData.find(t => t.id === team.id) || team;
    setSelectedTeam(fullTeamData);
    setShowManageMembersModal(true);
  };

  const handleEditTeam = (team) => {
    // Find the full team data from teamsData for editing
    const fullTeamData = teamsData.find(t => t.id === team.id) || team;
    setSelectedTeam(fullTeamData);
    setEditFormData({
  ...
  const calculateStats = () => {
    ...
    // Use global points data for accurate calculations
    const globalTeamsArray = Array.isArray(globalPointsData.teams) ? globalPointsData.teams : [];
    
    // Use dashboard data for accurate totals (not affected by pagination)
    const totalStudents = dashboardData?.totalStudents || studentsArray.length;
    const totalTeams = dashboardData?.totalTeams || teamsArray.length;
    const totalEvents = dashboardData?.totalEvents || eventsArray.length;
    ...
    const stats = {
      ...
      totalPoints: globalTeamsArray.reduce((sum, team) => sum + (team.points || 0), 0),
      averagePointsPerTeam: globalTeamsArray.length > 0 ? (globalTeamsArray.reduce((sum, team) => sum + (team.points || 0), 0) / globalTeamsArray.length).toFixed(1) : 0,
      topPerformingTeam: globalTeamsArray.length > 0 ? globalTeamsArray.reduce((max, team) => (team.points || 0) > (max.points || 0) ? team : max) : null,
      studentsByCategory: {
        hs: studentsArray.filter(s => s.category === 'hs').length,
        hss: studentsArray.filter(s => s.category === 'hss').length,
        general: studentsArray.filter(s => s.category === 'general').length
      },
      eventsByType: {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-gray-600 mb-2">Total Global Points</p>
                <p className="text-3xl font-bold text-gray-900 mb-2">
                  {stats.totalPoints}%
                </p>
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-blue-600 font-medium bg-blue-50 px-2 py-1 rounded-full">
                    📊 {stats.topPerformingTeam?.points || 0}% Global Points
                  </span>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(dashboardData?.pointsByTeam || []).slice(0, 6).map((team) => (
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{team.students} members</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-bold text-blue-600">{team.points}%</p>
                    <p className="text-xs text-gray-500">global points</p>
                  </div>
                </div>
                <div className="flex items-center justify-between">
                  <button
          ...
          {(dashboardData?.pointsByTeam || []).length > 6 && (
      ...
      {toast && (
        <div className={`fixed top-4 right-4 px-4 py-2 rounded shadow-lg z-50 ${toast.type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
          {toast.message}
          <button className="ml-2 text-white font-bold" onClick={() => setToast(null)}>×</button>
        </div>
      )}
    </div>
  );
