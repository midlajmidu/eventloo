...
def generate_program_details_executable(request, event_id):
    ...
    try:
        ...
        # Create executable content (self-contained HTML with embedded data)
        executable_content = f"""
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Eventloo - Program Details Report</title>
    <style>
        body {{
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="stats">
                ...
                <div class="stat-card">
                    <div class="stat-number">{programs.values('category').distinct().count()}</div>
                    <div class="stat-label">Categories</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">{event.start_date.strftime('%B %d, %Y') if event.start_date else 'TBD'}</div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{program.description or 'N/A'}</td>
                        </tr>
                        ''' for program in programs.filter(category=category)])}
                    </tbody>
                </table>
            </div>
            ''' for category in programs.values_list('category', flat=True).distinct()])}
        </div>
        
        <div class="footer">
            <p>Generated on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}</p>
            <p>Eventloo - School Event Management System</p>
        </div>
    </div>
</body>
</html>
        ...
        response = HttpResponse(executable_content, content_type='text/html')
        response['Content-Disposition'] = f'attachment; filename="Eventloo_Program_Details_{event.title}_{datetime.now().strftime("%Y%m%d")}.exe"'
        return response
...
def generate_complete_results_executable(request, event_id):
    ...
    try:
        event = Event.objects.get(id=event_id)
        results = ProgramResult.objects.filter(program__event=event).select_related('team', 'program').order_by('program__category', 'program__name', 'rank')
        ...
        for result in results:
            ...
            if program_name not in results_by_category[category]:
                results_by_category[category][program_name] = []
            results_by_category[category][program_name].append(result)
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Eventloo - Complete Results Report</title>
    <style>
        body {{
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            margin: 0;
        ...
        .rank-3 {{ background-color: #e2e3e5; }}
        .footer {{
            background-color: #f8f9fa;
            padding: 20px;
            text-align: center;
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="stats">
                ...
                <div class="stat-card">
                    <div class="stat-number">{len(results_by_category)}</div>
                    <div class="stat-label">Categories</div>
                </div>
                <div class="stat-card">
                    <div class="stat-number">{sum(len(programs) for programs in results_by_category.values())}</div>
            ...
            <div class="category-section">
                ...
                <div class="program-section">
                    ...
                    <table>
                        ...
                        <tbody>
                            {''.join([f'''
                            <tr class="rank-{min(result.rank, 3) if result.rank else 0}">
                                <td>{result.rank or 'N/A'}</td>
                                <td>{result.team.team_name if result.team else 'N/A'}</td>
                                <td>{result.points or 0}</td>
                                <td>{result.remarks or '-'}</td>
                            </tr>
                            ''' for result in sorted(program_results, key=lambda x: x.rank or 999)])}
                        </tbody>
                    </table>
                </div>
                ''' for program_name, program_results in programs.items()])}
            </div>
            ''' for category, programs in results_by_category.items()])}
        </div>
        ...
        # Return executable file
        response = HttpResponse(executable_content, content_type='text/html')
        response['Content-Disposition'] = f'attachment; filename="Eventloo_Complete_Results_{event.title}_{datetime.now().strftime("%Y%m%d")}.exe"'
        return response
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Eventloo - Participants Team Report</title>
    <style>
        body {{
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            margin: 0;
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="stats">
                ...
                <div class="stat-card">
                    <div class="stat-number">{sum(team.members.count() for team in teams)}</div>
            ...
            <div class="summary-section">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>Venue</td>
                            <td>{event.venue or 'N/A'}</td>
                        </tr>
                        <tr>
                            <td>Start Date</td>
                            <td>{event.start_date.strftime('%B %d, %Y') if event.start_date else 'N/A'}</td>
                        </tr>
                        <tr>
                            <td>End Date</td>
                            <td>{event.end_date.strftime('%B %d, %Y') if event.end_date else 'N/A'}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="summary-section">
                <div class="section-header">
                    <h3>Team-wise Participants and Programs</h3>
                </div>
                ...
                <div class="team-section">
                    ...
                    <div style="padding: 20px;">
                        <h5 style="margin: 0 0 15px 0; color: #495057;">Team Members:</h5>
                        <table>
                            <thead>
                                <tr>
                                    <th>Student Name</th>
                            ...
                            <tbody>
                                {''.join([f'''
                                <tr>
                                    <td>{member.get_full_name() if hasattr(member, 'get_full_name') else f"{member.first_name} {member.last_name}".strip() or member.name or f"Student {member.student_id}" if member.student_id else "Unknown Student"}</td>
                                    <td>{member.student_id if hasattr(member, 'student_id') and member.student_id else 'N/A'}</td>
                                    <td>{member.grade if hasattr(member, 'grade') and member.grade else 'N/A'}</td>
                                    <td>{member.section if hasattr(member, 'section') and member.section else 'N/A'}</td>
                                </tr>
                                ''' for member in team.members.all()])}
                            </tbody>
                        </table>
                        
                        <h5 style="margin: 25px 0 15px 0; color: #495057;">Program Participation:</h5>
                        <table>
                            <thead>
                                <tr>
                                    <th>Student Name</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{assignment.program.category}</td>
                                    <td>{assignment.program.venue or 'N/A'}</td>
                                    <td>{f"{assignment.program.start_time.strftime('%I:%M %p') if assignment.program.start_time else 'N/A'} - {assignment.program.end_time.strftime('%I:%M %p') if assignment.program.end_time else 'N/A'}"}</td>
                                </tr>
                                ''' for member in team.members.all() for assignment in member.program_assignments.filter(program__event=event).select_related('program')]) or f'''
                                <tr>
                                    <td colspan="5" class="no-data">No programs assigned to team members</td>
                                </tr>
                                '''}
                            </tbody>
                        </table>
                    ...
                    <div class="no-data">No members assigned to this team.</div>
                    '''}
                </div>
                ''' for team in teams])}
            </div>
        ...
        # Return executable file
        response = HttpResponse(executable_content, content_type='text/html')
        response['Content-Disposition'] = f'attachment; filename="Eventloo_Participants_Team_{event.title}_{datetime.now().strftime("%Y%m%d")}.exe"'
        return response
